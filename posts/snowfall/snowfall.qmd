---
title: <div style="display: flex; align-items: center; justify-content: center; gap: 20px; flex-wrap: wrap;">
    <img src="Alta_logo_dropshadow.jpg" alt="Alta Logo" style="height: 60px;">
    <span class="humans-learning-post">Snowfall at Alta, UT</span>
    <img src="uac_logo.svg" alt="Utah Avalanche Center Logo" style="height: 60px;">
  </div>
...
The new dataframe, `df3` is much longer, indeed. We now have `{r} paste0(nrow(df3), "rows and ", ncol(df3), " columns")`.

Before taking a first look at the data let's clean up a few more things -- change the "N" in the nino column to "Neutral" and make the months start with uppercase letters in the month column.
...
Finally, we want to remove the rain data, make sure it is numeric, remove rows of missing data, and ensure that our factor data has an order. 
...
The tooltip that pops up is not the useful out of the box. So, let's provide an additional parameter `geom_jitter()` called `text`, and then tell plotly to use that for the tooltip.

The tooltip pulls data from the season, month, amount, and nino columns and combines it using `paste0()`.
...
plot_ly_cap <- plot_ly |> 
  layout(annotations = list(
    ...
    list(x = 0,
        y = 1.08,
        text = "Alta gets a lot of snow, particularly from December to January",
        xref = "paper",
        yref = "paper",
        xanchor = "left",
        showarrow = FALSE,
